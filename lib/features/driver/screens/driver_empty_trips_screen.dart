...
class DriverEmptyTripsScreen extends StatefulWidget {
  ...
  const DriverEmptyTripsScreen({super.key, this.onNeedHistoryTab});

  @override
  State<DriverEmptyTripsScreen> createState() => _DriverEmptyTripsScreenState();
}

class _DriverEmptyTripsScreenState extends State<DriverEmptyTripsScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadMyTrips() async {
    ...
    debugPrint('📍 DriverEmptyTripsScreen loaded ${trips.length} active trips');
    setState(() {
      myTrips = trips;
      _isLoading = false;
    });
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              // YÊU CẦU CHỜ DUYỆT
              if (trip.joinedShippers.any((s) => s.status == 'pending'))
                Container(
                  padding: const EdgeInsets.all(12),
                  decoration: BoxDecoration(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('⏳ Yêu cầu chờ duyệt', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                      const SizedBox(height: 12),
                      ...trip.joinedShippers.where((s) => s.status == 'pending').map((shipper) {
                        return Container(
                                ...
                                children: [
                                  ...
                                  Expanded(
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text(shipper.shipperName, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                                        Text('${shipper.cargoWeight} tấn - ${shipper.cargoType}', style: const TextStyle(fontSize: 13, color: Colors.grey)),
                                      ],
                                    ),
                                  ),
                                ],
                              ),
                              if (shipper.fromDetail.isNotEmpty || shipper.toDetail.isNotEmpty) ...[
                                const SizedBox(height: 8),
                                const Divider(),
                                if (shipper.fromDetail.isNotEmpty)
                                  Text('📍 Nhận: ${shipper.fromDetail}', style: const TextStyle(fontSize: 12)),
                                if (shipper.toDetail.isNotEmpty)
                                  Text('📍 Giao: ${shipper.toDetail}', style: const TextStyle(fontSize: 12)),
                              ],
                              const SizedBox(height: 8),
                              Text('Giá: ${shipper.price}đ', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.red, fontSize: 15)),
                              const SizedBox(height: 12),
                              Row(
                                children: [
                                  Expanded(
                                    child: ElevatedButton.icon(
                                      ...
                                      onPressed: () async {
                                        await EmptyTripService.approveJoinRequest(trip.id, shipper.shipperId);
                                        if (mounted) {
                                          Navigator.pop(context);
                                          await _loadMyTrips();
                                          ScaffoldMessenger.of(context).showSnackBar(
                                            SnackBar(content: Text('✅ Đã duyệt ${shipper.shipperName}')),
                                          );
                                        }
                                      },
                                    ),
                                  ...
                                  Expanded(
                                    child: ElevatedButton.icon(
                                      ...
                                      onPressed: () async {
                                        await EmptyTripService.rejectJoinRequest(trip.id, shipper.shipperId);
                                        if (mounted) {
                                          Navigator.pop(context);
                                          await _loadMyTrips();
                                          ScaffoldMessenger.of(context).showSnackBar(
                                            SnackBar(content: Text('❌ Đã từ chối ${shipper.shipperName}')),
                                          );
                                        }
                                      },
                                    ),
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        const Text('✅ Chủ hàng đã duyệt', style: TextStyle(fontWeight: FontWeight.bold)),
                        Chip(
                          label: Text('${trip.joinedShippers.where((s) => s.status == "approved").length}/${trip.maxShippers}'),
                          backgroundColor: trip.hasAvailableSlots ? Colors.green[200] : Colors.red[200],
                        ),
                      ],
                    ),
                    const SizedBox(height: 8),
                    if (trip.joinedShippers.where((s) => s.status == 'approved').isEmpty)
                      const Text('Chưa có chủ hàng nào được duyệt', style: TextStyle(color: Colors.grey, fontStyle: FontStyle.italic))
                    else
                      Column(
                        children: trip.joinedShippers.where((s) => s.status == 'approved').map((shipper) {
                          return Padding(
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        '${shipper.cargoWeight} tấn - ${shipper.cargoType}',
                  ...
                  Expanded(
                    child: ElevatedButton(
                      style: ElevatedButton.styleFrom(
                        backgroundColor: (trip.joinedShippers.where((s) => s.status == 'approved').length < trip.maxShippers) 
                            ? Colors.grey 
                            : AppColors.primary,
                      ),
                      onPressed: (trip.joinedShippers.where((s) => s.status == 'approved').length < trip.maxShippers)
                          ? () {
                              Navigator.pop(context);
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text('Chưa đủ ${trip.maxShippers} chủ hàng được duyệt - Hiện có ${trip.joinedShippers.where((s) => s.status == "approved").length}'),
                                  backgroundColor: Colors.blue,
                                ),
                              );
                            }
                          : () async {
                              // Chuyến đã đầy → cập nhật status + cập nhật orders
                              await EmptyTripService.startDelivering(trip.id);
                              if (mounted) {
                                Navigator.pop(context);
                                await _loadMyTrips();
                                // Call callback to switch to History tab
                                widget.onNeedHistoryTab?.call();
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text('✅ Bắt đầu giao hàng! Kiểm tra tại Lịch sử chuyến.'),
                      ...
                      child: Text(
                        (trip.joinedShippers.where((s) => s.status == 'approved').length < trip.maxShippers) 
                            ? 'Chưa đủ người (${trip.joinedShippers.where((s) => s.status == "approved").length}/${trip.maxShippers})' 
                            : 'Bắt đầu giao hàng',
