...
class _MatchCargoScreenState extends State<MatchCargoScreen> {
  ...
  Future<void> _showTripDetails(EmptyTrip trip) async {
    ...
    final hasJoined =
        trip.joinedShippers.any((s) => s.shipperId == currentShipperId);

    debugPrint('🔍 Current Shipper ID: $currentShipperId');
    debugPrint(
        '🔍 Joined Shippers: ${trip.joinedShippers.map((s) => s.shipperId).toList()}');
  ...
  void _showJoinDialog(EmptyTrip trip) {
    ...
    final totalPrice =
        double.tryParse(trip.proposedPrice.replaceAll(RegExp(r'[^0-9]'), '')) ??
            0;
    ...
    if (totalCapacity > 0) {
      ...
      debugPrint(
          '   - Suggested price: $suggestedPrice VNĐ (${totalPrice}/${totalCapacity}*$selectedWeight)');
    }
            ...
            return SingleChildScrollView(
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      Expanded(
                        ...
                        child: Slider(
                          ...
                          divisions:
                              (availableCapacity / 0.1).round().clamp(1, 1000),
                            ...
                            setState(() {
                              ...
                              if (totalCapacity > 0) {
                                final suggestedPrice =
                                    (totalPrice / totalCapacity) * value;
                                priceController.text =
                                    suggestedPrice.toStringAsFixed(0);
                                debugPrint(
                                    '🔄 Slider changed: weight=$value tấn → price=${suggestedPrice.toStringAsFixed(0)} VNĐ');
                              }
                            });
                          },
                        ),
                          ...
                          onChanged: (value) {
                            ...
                            if (parsed != null &&
                              ...
                              setState(() {
                                ...
                                if (totalCapacity > 0) {
                                  final suggestedPrice =
                                      (totalPrice / totalCapacity) * parsed;
                                  priceController.text =
                                      suggestedPrice.toStringAsFixed(0);
                                  debugPrint(
                                      '✏️ Manual input: weight=$parsed tấn → price=${suggestedPrice.toStringAsFixed(0)} VNĐ');
                                }
                              });
                            }
                          },
                  ...
                  Text('Tối đa: ${availableCapacity.toStringAsFixed(1)} tấn',
                      style: const TextStyle(fontSize: 12, color: Colors.grey)),
                  const SizedBox(height: 16),

                  // Địa điểm nhận hàng with LocationPickerWidget
                  LocationPickerWidget(
                    ...
                    onLocationSelected: (province, district, coordinates) {
                      ...
                      debugPrint(
                          '📍 From location: $province${district != null ? ', $district' : ''} - Coordinates: $coordinates');
                    },
                  ),
                  if (fromProvince != null && fromDistrict != null)
                    Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'Đã chọn: $fromDistrict, $fromProvince',
                  ...
                  // Địa điểm giao hàng with LocationPickerWidget
                  LocationPickerWidget(
                    ...
                    onLocationSelected: (province, district, coordinates) {
                      ...
                      debugPrint(
                          '📍 To location: $province${district != null ? ', $district' : ''} - Coordinates: $coordinates');
                    },
                  ),
                  if (toProvince != null && toDistrict != null)
                    Padding(
                      padding: const EdgeInsets.only(top: 4, bottom: 8),
                      child: Row(
                        children: [
            ...
            onPressed: () async {
              ...
              if (cargoTypeController.text.trim().isEmpty) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                      content: Text('Vui lòng nhập loại hàng!'),
              ...
              if (fromProvince == null ||
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                      content: Text('Vui lòng chọn địa điểm nhận hàng!'),
                      backgroundColor: Colors.orange),
                );
                return;
              }
              if (toProvince == null ||
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                      content: Text('Vui lòng chọn địa điểm giao hàng!'),
                      backgroundColor: Colors.orange),
                );
                return;
              }
              ...
              // Build fromDetail and toDetail strings
              final fromDetail = '$fromDistrict, $fromProvince';
              final toDetail = '$toDistrict, $toProvince';

              debugPrint('📦 Sending join request with coordinates:');
              debugPrint('   From: $fromDetail → $fromCoordinates');
              debugPrint('   To: $toDetail → $toCoordinates');
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(success
                      ? '✅ Gửi yêu cầu thành công! Chờ tài xế duyệt.'
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Ghép hàng tiết kiệm',
      ...
      body: _isLoading
          ...
          : availableTrips.isEmpty
              ? const Center(
                  child: Text('Hiện không có chuyến ghép hàng nào',
                                ...
                                children: [
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      Chip(
                                        label: Text(
                                            '${trip.joinedShippers.length}/${trip.maxShippers} slots'),
                                        backgroundColor: trip.hasAvailableSlots
                                            ? Colors.orange[100]
                                            : Colors.red[100],
                                      ),
                                    ],
  ...
  Widget _buildTripInfoSection(EmptyTrip trip) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            _detailRow('Thời gian nhận:',
                DateFormat('dd/MM HH:mm').format(trip.pickupTime)),
            _detailRow('Thời gian giao:',
                DateFormat('dd/MM HH:mm').format(trip.deliveryTime)),
          ],
        ),
      );
        ...
        child: Column(
          ...
          children: [
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                const Text('👥 Chủ hàng đã tham gia',
                    style: TextStyle(fontWeight: FontWeight.bold)),
                Chip(
                  label:
                      Text('${trip.joinedShippers.length}/${trip.maxShippers}'),
                  backgroundColor: trip.hasAvailableSlots
                      ? Colors.orange[200]
                      : Colors.red[200],
                ),
            ...
            if (trip.joinedShippers.isEmpty)
              const Text('Chưa có chủ hàng nào tham gia',
                  style: TextStyle(
                      color: Colors.grey, fontStyle: FontStyle.italic))
            else
              Column(
                children: trip.joinedShippers
                    .map((s) =>
                        Text('- ${s.shipperName} (${s.cargoWeight} tấn)'))
                    .toList(),
              ),
          ],
        ),
  ...
  Widget _buildFillProgress(EmptyTrip trip) => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Tỉ lệ lấp đầy (theo khối lượng)',
              style: TextStyle(fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          ClipRRect(
            borderRadius: BorderRadius.circular(8),
          ...
          Align(
            ...
            child: Text('${trip.fillPercentage.toStringAsFixed(1)}%',
                style: const TextStyle(
  ...
  Widget _buildActionButton(
      EmptyTrip trip, bool hasJoined, String currentShipperId) {
  ...
  Widget _detailRow(String label, String value, {bool isAddress = false}) =>
      Padding(
        ...
        child: Row(
          ...
          children: [
            ...
            Expanded(
                child: Text(value,
                    style: TextStyle(
                        color: isAddress ? AppColors.primary : null))),
          ],
        ),
      );
}
